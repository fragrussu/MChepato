#### Simulate MRI signals for one specific microstructural scenario for multiple gradient orientations per b-value, without averaging them
...
if __name__ == "__main__":
	...
	parser.add_argument('gradfile', help='text file storing gradient directions for each non-zero b-value as a matrix of size Nx3 (space-separated)')
	...
	grads = np.loadtxt(args.gradfile)
	outdir = args.outdir
	...
	for ii in range(0,len(ustruct_list)):
		...
		processinglist.append([bvalseq,deltaseq,Deltaseq,grads,Nspins,Nsteps,Tdur,ustruct_files,ii])     # Append each slice list and create a longer list of MRI slices whose processing will run in parallel
