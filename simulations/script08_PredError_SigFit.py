## Show prediction error on test set for SigFit estimation
import pickle as pk
import numpy as np
import matplotlib
...
bmax = 1000.0
SNR = 20   # SNR to plot; select from np.inf, 80, 40, 20
...
for nn in range(0,len(gDur_vals)):
	...
	h = open('./results_gDur{}_gSep{}_bmin100.0_bmax{}_Nb7/syn_d{}_D{}_bmin100.0_bmax{}_Nbval7.SigFit.LD0pred.test.ivim.SNR{}.bin'.format(delta,Delta,bmax,delta,Delta,bmax,SNR),'rb')
	...
	plt.yticks(ticks=[-1.2 , -0.8, -0.4,  0.0,  0.4,  0.8, 1.0, 1.2, 1.4, 1.6])
...
# Show
plt.savefig('script08_PredError.SigFit.bmax{}.SNR{}.900dpi.png'.format(bmax,SNRstrfile),dpi=900)
plt.show()
